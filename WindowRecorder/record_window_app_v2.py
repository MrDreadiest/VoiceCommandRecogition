...
class App(ctk.CTk):
    ...
    def initialize_variable(self):
        ...
        self.rec_param_frames_per_buffer: int = FRAMES_PER_BUFFER
        self.rec_param_frame_rate = FRAME_RATE
        self.rec_param_channels = CHANNELS
        self.rec_param_format = FORMAT
        ...
        self.input_device_0_max_input_channels = CHANNELS
        ...
        self.command_list = ["01-Odrzuć broń", "02-Obróć się", "03-Na kolana", "04-Gleba", "05-Ręce na głowę","11-Odrzuć broń podobne","12-Obróć się podobne","13-Na kolana podobne","14-Gleba podobne", "15-Ręce na głowę podobne", "06-tlo"]
    ...
    def initialize_gui(self):
        ...
        self.gui_countdown_value_label = ctk.CTkLabel(self.gui_countdown_frame, text=self.record_countdown,
        ...
        # 0 row
        self.gui_header_0_label = ctk.CTkLabel(self.gui_settings_side_bar_frame, text=self.gui_header_0_name,
                                               font=ctk.CTkFont(size=24, weight="bold"))
        self.gui_header_0_label.grid(row=0, column=0, columnspan=5, padx=10, pady=20)

        # 0 row

        self.gui_input_device_label = ctk.CTkLabel(self.gui_settings_side_bar_frame, text="Input device",
                                                   font=ctk.CTkFont(size=12, weight="bold"))
        self.gui_input_device_label.grid(row=1, column=0, columnspan=4, padx=(10, 5), pady=5, sticky="nsw")

        self.gui_input_device_refresh_button = ctk.CTkButton(self.gui_settings_side_bar_frame, text="↺", width=50,
                                                             command=self.update_input_devices)
        self.gui_input_device_refresh_button.grid(row=1, column=4, padx=(5, 10), pady=5, sticky="nse")

        # 5 row

        self.gui_input_device_0_label = ctk.CTkLabel(self.gui_settings_side_bar_frame, text="1.",
                                                     font=ctk.CTkFont(size=12, weight="bold"))
        self.gui_input_device_0_label.grid(row=2, column=0, padx=(10, 5), pady=5, sticky="w")
        self.gui_input_device_0_combobox = ctk.CTkComboBox(self.gui_settings_side_bar_frame,
                                                           values=self.input_device_list,
                                                           command=self.input_device_0_combobox_callback)
        self.gui_input_device_0_combobox.grid(row=2, column=1, columnspan=4, padx=(5, 10), pady=5, sticky="nsew")

        # 6 row
        self.gui_header_1_label = ctk.CTkLabel(self.gui_settings_side_bar_frame, text=self.gui_header_1_name,
                                               font=ctk.CTkFont(size=24, weight="bold"))
        self.gui_header_1_label.grid(row=6, column=0, columnspan=5, padx=10, pady=20)
        # 7 row
        self.gui_speaker_sex_segbutton = ctk.CTkSegmentedButton(self.gui_settings_side_bar_frame,
                                                                values=self.speaker_sex_type_list,
                                                                command=self.input_sex_type_segmented_button_callback)
        self.gui_speaker_sex_segbutton.grid(row=7, column=0, columnspan=5, padx=10, pady=5, sticky="nsew")
        # 8 row
        self.gui_speaker_number_label = ctk.CTkLabel(self.gui_settings_side_bar_frame, text="Number",
                                                     font=ctk.CTkFont(size=12, weight="bold"))
        self.gui_speaker_number_label.grid(row=8, column=0, columnspan=2, padx=(10, 5), pady=5, sticky="w")

        self.gui_speaker_prev_button = ctk.CTkButton(self.gui_settings_side_bar_frame, text="<", width=50,
                                                     command=self.input_prev_speaker_button_callback)
        self.gui_speaker_prev_button.grid(row=8, column=2, padx=(10, 5), pady=5, sticky="w")

        self.gui_speaker_number_value_label = ctk.CTkLabel(self.gui_settings_side_bar_frame, text="0000",
                                                           font=ctk.CTkFont(size=12))
        self.gui_speaker_number_value_label.grid(row=8, column=3, padx=(5, 10), pady=5, sticky="ew")

        self.gui_speaker_next_button = ctk.CTkButton(self.gui_settings_side_bar_frame, text=">", width=50,
                                                     command=self.input_next_speaker_button_callback)
        self.gui_speaker_next_button.grid(row=8, column=4, padx=(5, 10), pady=5, sticky="e")
        # 9 row
        self.gui_command_attempt_label = ctk.CTkLabel(self.gui_settings_side_bar_frame, text="Attempt",
                                                      font=ctk.CTkFont(size=12, weight="bold"))
        self.gui_command_attempt_label.grid(row=9, column=0, columnspan=2, padx=(10, 5), pady=5, sticky="w")

        self.gui_command_attempt_prev_button = ctk.CTkButton(self.gui_settings_side_bar_frame, text="<", width=50,
                                                             command=self.input_prev_command_attempt_button_callback)
        self.gui_command_attempt_prev_button.grid(row=9, column=2, padx=(10, 5), pady=5, sticky="w")

        self.gui_command_attempt_value_label = ctk.CTkLabel(self.gui_settings_side_bar_frame, text="0000",
                                                            font=ctk.CTkFont(size=12))
        self.gui_command_attempt_value_label.grid(row=9, column=3, padx=(5, 10), pady=5, sticky="ew")

        self.gui_command_attempt_next_button = ctk.CTkButton(self.gui_settings_side_bar_frame, text=">", width=50,
                                                             command=self.input_next_command_attempt_button_callback)
        self.gui_command_attempt_next_button.grid(row=9, column=4, padx=(5, 10), pady=5, sticky="e")
        # 10 row
        self.gui_command_label = ctk.CTkLabel(self.gui_settings_side_bar_frame, text="Command",
                                              font=ctk.CTkFont(size=12, weight="bold"))
        self.gui_command_label.grid(row=10, column=0, columnspan=2, padx=(10, 5), pady=5, sticky="w")

        self.gui_command_combobox = ctk.CTkComboBox(self.gui_settings_side_bar_frame, values=self.command_list,
                                                    command=self.input_command_combobox_callback)
        self.gui_command_combobox.grid(row=10, column=2, columnspan=3, padx=(5, 10), pady=5, sticky="nsew")
        # 11 row
        self.gui_header_2_label = ctk.CTkLabel(self.gui_settings_side_bar_frame, text=self.gui_header_2_name,
                                               font=ctk.CTkFont(size=24, weight="bold"))
        self.gui_header_2_label.grid(row=11, column=0, columnspan=5, padx=10, pady=20, sticky="nsew")
        # 12 row
        self.gui_rec_type_segbutton = ctk.CTkSegmentedButton(self.gui_settings_side_bar_frame,
                                                             values=self.record_type_list,
                                                             command=self.input_rec_type_segmented_button_callback)
        self.gui_rec_type_segbutton.grid(row=12, column=0, columnspan=5, padx=10, pady=5, sticky="nsew")
        # 13 row
        self.gui_rec_window_time_label = ctk.CTkLabel(self.gui_settings_side_bar_frame, text="Time [s]",
                                                      font=ctk.CTkFont(size=12, weight="bold"))
        ...
        # 14 row
        self.gui_record_path_label = ctk.CTkLabel(self.gui_settings_side_bar_frame, text="Record path",
                                                  font=ctk.CTkFont(size=12, weight="bold"))
        self.gui_record_path_label.grid(row=14, column=0, columnspan=5, padx=10, pady=2, sticky="nsew")
        # 15 row
        self.gui_record_path_value_label = ctk.CTkLabel(self.gui_settings_side_bar_frame,
                                                        text="data/command/person_sex_attempt.wav",
                                                        font=ctk.CTkFont(size=12))
        self.gui_record_path_value_label.grid(row=15, column=0, columnspan=5, padx=10, pady=2, sticky="nsew")
        # 16 row
        self.gui_record_time_label = ctk.CTkLabel(self.gui_settings_side_bar_frame, text="Time m : s : ms",
                                                  font=ctk.CTkFont(size=12, weight="bold"))
        self.gui_record_time_label.grid(row=16, column=0, columnspan=5, padx=10, pady=2, sticky="nsew")
        # 17 row
        self.gui_record_time_value_label = ctk.CTkLabel(self.gui_settings_side_bar_frame, text="00 : 00 : 0000",
                                                        font=ctk.CTkFont(size=12))
        self.gui_record_time_value_label.grid(row=17, column=0, columnspan=5, padx=10, pady=2, sticky="nsew")
        # 18 row
        self.gui_record_button = ctk.CTkButton(self.gui_settings_side_bar_frame, text="Record",
                                               command=self.input_record_button_callback)
        self.gui_record_button.grid(row=18, column=0, columnspan=5, padx=10, pady=2, sticky="nsew")
        # 19 row
        self.gui_save_button = ctk.CTkButton(self.gui_settings_side_bar_frame, text="Save",
                                             command=self.input_save_button_callback)
        self.gui_save_button.grid(row=19, column=0, columnspan=5, padx=10, pady=10, sticky="nsew")

        # 20 row
        self.gui_play_button = ctk.CTkButton(self.gui_settings_side_bar_frame, text="Play",
                                             command=self.input_play_button_callback)
    ...
    def get_input_device_channels(self, device_id):

        audio = pyaudio.PyAudio()
        number_of_channels = audio.get_device_info_by_host_api_device_index(0, device_id).get('maxInputChannels')
        audio.terminate()
    ...
    def get_file_name_split(self, file_path: str) -> tuple[str, str]:
        name = os.path.normpath(file_path).split(os.sep)[-1].split('.')[0]
        extension = os.path.normpath(file_path).split(os.sep)[-1].split('.')[-1]
    ...
    def gui_update_record_time_label(self):
        frames = len(b"".join(self.rec_data_0_frames)) / self.input_device_0_max_input_channels
        self.gui_record_time_value_label.configure(
            text=f"{frames / (pyaudio.get_sample_size(self.rec_param_format) * self.rec_param_frame_rate):.4f}")
    ...
    def input_device_0_combobox_callback(self, choice):
        ...
        self.input_device_0_max_input_channels = self.get_input_device_channels(self.input_device_0_index)
    ...
    def input_play_button_default(self):
        self.gui_play_button.configure(fg_color="#2B719E")
        self.gui_play_button.configure(hover_color="#144870")
        self.gui_play_button.configure(text="Play")

    #
    def input_play_button_red(self):
        self.gui_play_button.configure(fg_color="#FF0040")
        self.gui_play_button.configure(hover_color="#8b0000")
        self.gui_play_button.configure(text="Pause")
    ...
    def record_free(self):
        ...
        stream = audio.open(
            ...
            channels=self.input_device_0_max_input_channels,
    ...
    def input_play_button_callback(self):

        if self.play_audio_flag == False:
            self.play_audio_thread = threading.Thread(target=self.play_record)
    ...
    def play_record(self):
        ...
        if len(self.rec_data_0_frames) > 0:
            ...
            while True:
                if iterator >= len(self.rec_data_0_frames):
                    break

                stream.write(b"".join(self.rec_data_0_frames[
                                      iterator * self.rec_param_frames_per_buffer:(iterator + 1) * self.rec_param_frames_per_buffer]))
    ...
    def input_save_button_callback(self):
        ...
        if len(self.rec_data_0_frames) > 0:
            ...
            head, tail = os.path.split(self.record_file_path)
            name, extension = self.get_file_name_split(tail)

            result = np.frombuffer(b"".join(self.rec_data_0_frames), dtype=np.int32)
            result.shape = -1, 2
            result = result.T

            for i in range(self.input_device_0_max_input_channels):

                path_temp = os.path.normpath(self.record_file_path).split(os.sep)
                path_temp[-1] = name + f"_c{i}" + '.' + extension
                path_temp = os.path.join(*path_temp)
                path_temp = os.path.join(os.getcwd(), path_temp)

                head, tail = os.path.split(path_temp)

                if os.path.exists(head) == False:
                    os.makedirs(head)

                sf.write(file=path_temp, data=result[i], samplerate=self.rec_param_frame_rate, subtype="PCM_16")
    ...
    def plot_wave(self, frames_len, frames: bytes):
        ...
        result.shape = -1, self.input_device_0_max_input_channels
        result = result.T

        fig, ax = plt.subplots(ncols=self.input_device_0_max_input_channels, sharex=True)

        if self.input_device_0_max_input_channels > 1:
            for i in range(0, self.input_device_0_max_input_channels):
                ax[i].plot(result[i], linewidth=0.1)
                ax[i].set_ylabel("Signal wave amplitude")
                ax[i].set_xlabel("Frames")
        else:
            ax.plot(result[0], linewidth=0.1)
            ax.set_ylabel("Signal wave amplitude")
            ax.set_xlabel("Frames")

        canvas = FigureCanvasTkAgg(fig, master=self.gui_wave_form_graph_frame)
        canvas.draw()
        ...
        _nfft = 256
        _noverlap = 128
        fig2, ax2 = plt.subplots(ncols=self.input_device_0_max_input_channels, sharex=True)

        if self.input_device_0_max_input_channels > 1:
            for i in range(0, self.input_device_0_max_input_channels):
                Pxx, freqs, bins, im = ax2[i].specgram(result[i], NFFT=_nfft, Fs=_fs, noverlap=_noverlap)
                ax2[i].set_title("Spectrogram")
                ax2[i].set_ylabel("Frequency [Hz]")
                ax2[i].set_xlabel("Frames")
        else:
            Pxx, freqs, bins, im = ax2.specgram(result[0], NFFT=_nfft, Fs=_fs, noverlap=_noverlap )
            ax2.set_title("Spectrogram")
            ax2.set_ylabel("Frequency [Hz]")
            ax2.set_xlabel("Frames")

        canvas2 = FigureCanvasTkAgg(fig2, master=self.gui_spectrogram_graph_frame)
        canvas2.draw()
